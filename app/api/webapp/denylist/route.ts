...
import { addDenylistUser, removeDenylistUser } from "../../../../src/config/store";
...
export async function POST(req: Request): Promise<Response> {
  ...
  let updated;
  if (body.action === "add") {
    updated = await addDenylistUser(auth.storage.configStorage, auth.chatId, userId);
  } else if (body.action === "remove") {
    updated = await removeDenylistUser(auth.storage.configStorage, auth.chatId, userId);
  } else {
